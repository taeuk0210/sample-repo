...
repository -> `Projects` 탭 -> `+ New project` 버튼 클릭 후 새로운 프로젝트를 생성합니다.  
...
코드를 작성하고 커밋합니다. 이때 나중에 git log를 볼 때 무엇을 왜 했는지 한눈에 알 수 있도록 커밋 메세지를 작성합니다.
...
그 버튼을 누르고 병합할 브랜치 설정(방향 중요), 제목과 내용을 작성한 뒤 오른쪽 사이드바에서 리뷰어를 지정하고 이슈와 마찬가지로 라벨, 프로젝트, 마일스톤을 연결한 후 Create pull request를 클릭합니다.
...
리뷰어들의 검토 후 피드백을 반영하여 수정 커밋을 올립니다.  
모든 조건이 충족됐다면 `Merge pull request` 를 클릭하여 병합을 수행합니다.  
이때 동시에 연결된 이슈(#7)가 닫히고 마일스톤의 진행률바가 올라갑니다.
